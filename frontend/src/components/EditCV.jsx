...
const ALLOWED_SECTIONS = ["profiles", "work", "volunteer", "education", "awards", "certificates", "publications", "projects"];
...
function renderFields(data, path, handleChange, handleAddItem, handleRemoveItem) {
        ...
        if (typeof value === "object" && !Array.isArray(value)) {
            return (
                <fieldset key={newPath} className="border p-3 mb-3 bg-white shadow-sm rounded">
                    <legend className="fw-bold text-uppercase"><h3>{label}</h3></legend>
                    {renderFields(value, newPath, handleChange, handleAddItem, handleRemoveItem)}
                </fieldset>
            );
        // If its an array, render the title and call renderFields for each item
            ...
            return (
                <div key={newPath} className="mb-3">
                    <label className="fw-bold text-capitalize"><h3>{label}</h3></label>
                    {value.map((item, index) =>
                        typeof item === "object" ? (
                            <fieldset key={`${newPath}[${index}]`} className="border p-3 bg-white shadow-sm rounded">
                                ...
                                {renderFields(item, `${newPath}[${index}]`, handleChange, handleAddItem, handleRemoveItem)}
                                <button className="btn btn-danger mt-2" onClick={(e) => handleRemoveItem(newPath, index, e)}>-</button>
                            </fieldset>
                        ) : (
                            <input
                                key={`${newPath}[${index}]`}
                    ...
                    <button className="btn btn-success mt-2" onClick={(e) => handleAddItem(newPath, template, e)}>+</button>
                </div>
            );
